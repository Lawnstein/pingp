...
public class PingpApplication {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 1) {
        ...
        } else if ("upload".equalsIgnoreCase(args[0]) || "up".equalsIgnoreCase(args[0]) || "upsyn".equalsIgnoreCase(args[0])) {
            if (args.length >= 2 && "help".equals(args[1])) {
                System.out.println("up|upsyn [ip [port [path [max-threads] ] ] ]");
                return;
            }
            ...
            if ("upload".equalsIgnoreCase(args[0]) || "up".equalsIgnoreCase(args[0])) {
                upload(ip, port, path, maxThreads);
            } else if ("upsyn".equalsIgnoreCase(args[0])) {
                upsyn(ip, port, path, maxThreads);
            }

        } else if ("download".equalsIgnoreCase(args[0]) || "dw".equalsIgnoreCase(args[0]) || "dwsyn".equalsIgnoreCase(args[0])) {
            if (args.length >= 2 && "help".equals(args[1])) {
                System.out.println("dw|dwsyn [ip [port [path [max-threads] ] ] ]");
                return;
            }
            ...
            if ("download".equalsIgnoreCase(args[0]) || "dw".equalsIgnoreCase(args[0])) {
                download(ip, port, path, maxThreads);
            } else if ("dwsyn".equalsIgnoreCase(args[0])) {
                dwsyn(ip, port, path, maxThreads);
            }
    ...
    public static void upsyn(String ip, int port, String path, int maxThreads) {
        ClientProperties prop = applicationContext.getBean(ClientProperties.class);
        TcpClient client = new TcpClient(ip, port, path, maxThreads, prop);
        client.upsyn();
    }
    ...
    public static void dwsyn(String ip, int port, String path, int maxThreads) {
        ClientProperties prop = applicationContext.getBean(ClientProperties.class);
        TcpClient client = new TcpClient(ip, port, path, maxThreads, prop);
        client.dwsyn();
    }
